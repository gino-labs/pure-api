...
scriptlog.write_log("Create snapshots for every file system and wait 30 seconds.", show_output=True)

# Demote / Disable each file system on Legacy (Handle exception: non-replication snapshot error, skip demotion)
scriptlog.write_log("Demote Legacy filesystems, handle non replication snapshots.", show_output=True)
...
scriptlog.write_log("Delete replica links on Legacy.", show_output=True)
...
scriptlog.write_log(f"Run ansible playbook to handle remounting nfs clients, pass egrep string: {production_ips}", show_output=True)
