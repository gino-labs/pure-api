...
class FlashBladeAPI:
    ...
    def post_bucket_replica_link(self, bucket, remote_credential_id, payload):
        url = self.baseurl + f"bucket-replica-links?local_bucket_names={bucket}&remote_bucket_names={bucket}&remote_credential_names={remote_credential_id}"
        msg = f"bucket replica link: {bucket} with remote credential ID: {remote_credential_id}"
        data = self.REST_Request("post", url, msg, payload=payload)
